...
class Qwen_LongLLM(BaseLLM):
    ...
    def chat(self, text, history=None, temperature=0.2, max_tokens=2048):
        try:
            ...
            # 确保每个对话历史记录项是符合格式的字典
            if not conversation_history or 'role' not in conversation_history[-1]:
                conversation_history = [{"role": "system", "content": "You are a helpful assistant."}] + conversation_history
